...
HOSHIYOMI公式Xアカウント向けに、当日の天体イベントを計算し、投稿文を作成して1日4回自動投稿するサーバーレス構成です。朝は12星座ランキング、昼は総合運・恋愛運・金運・仕事運の独立ランキング、夜は12星座別の振り返りを画像付きスレッド投稿します。GitHub Actionsだけで動きます。
...
| JST | UTC cron | 内容 |
|---|---:|---|
| 00:17 | `17 15 * * *` | 日付が変わった直後の今日の星 |
| 08:17 | `17 23 * * *` | 今日の星の流れと12星座別の運気・やること |
| 12:47 | `47 3 * * *` | 星の位置から見る総合運・恋愛運・金運・仕事運ランキング |
| 22:17 | `17 13 * * *` | 今日の星のふり返りと12星座別の受け止め方 |

GitHub Actionsは指定時刻ぴったりに起動せず、特に毎時00分台は大きく遅れることがあります。そのためX投稿は00分を避け、予約投稿では実行された時刻から推定せず、cronごとに `midnight` / `morning` / `noon` / `night` を明示して渡しています。`midnight` 投稿がJST 01:00以降に起動した場合は、「日付が変わった直後」の投稿として不自然になるためスキップします。
...
Instagramはテキストだけでは投稿できないため、`instagram_post.py` が1080×1350の画像カードを生成し、Supabase Storageへアップロードした公開URLをInstagram Graph APIへ渡します。朝と夜はXの1投稿目と同じ設計の12星座ランキング画像、昼は総合運・恋愛運・金運・仕事運の4枚カルーセル、日付変更直後は「今日の星図」風カードを使います。ストーリーズは `instagram_story.py` が1080×1920の専用画像を作り、`media_type=STORIES` で投稿します。
...
`morning` / `night` は5投稿のスレッドとして出力します。`morning` は1件目に12星座ランキング画像を添付し、2〜5件目で3星座ずつの運気とやるべきことを投稿します。`night` は1件目に12星座ランキング画像を添付し、2〜5件目で3星座ずつの振り返りと、できなかった時の受け止め方を投稿します。

`noon` は総合運、恋愛運、金運、仕事運の4本の独立スレッドとして出力します。各スレッドの1件目に全12星座ランキング画像を添付し、その後の4投稿で、順位順に3星座ずつ詳しいコメントを投稿します。

X投稿は日付・月星座・投稿スロットを元に文体と表現パターンを切り替えます。Claude APIを使う場合も「導入文・語尾・構文が前回と同じに見えないこと」をプロンプトで明示しています。API未設定時のテンプレート投稿でも、同じ星座配置が数日続いた時に文言が固定されにくい設計です。

Instagramの `morning` / `night` は1枚画像+長めのキャプションで、12星座すべての運気/振り返りを掲載します。`noon` は総合運・恋愛運・金運・仕事運の4枚カルーセルで、キャプションにも各テーマの全順位を掲載します。投稿時刻はXの少し後になるよう、08:05 / 12:35 / 22:05 JSTにしています。

Instagram Storiesは08:10 / 12:40 / 22:10 JSTに投稿します。朝は「12星座別の運気はフィード投稿へ」、夜は「12星座別の振り返りはフィード投稿へ」という導線を画像内に入れます。API経由ではリンクスタンプやアンケートなどのInstagram純正スタンプは付けず、画像内テキストで誘導します。
...
DRY_RUN=1 python instagram_post.py noon
DRY_RUN=1 python instagram_story.py morning
DRY_RUN=1 python instagram_story.py night
```
...
1. Actionsタブを開きます。
2. `HOSHIYOMI Instagram story auto post` を選択します。
3. `Run workflow` を押します。
4. `dry_run` は最初は `1` のまま実行します。
5. 必要なら `slot` に `morning` / `noon` / `night` を入力します。
6. ログにストーリー画像パスが出ることを確認します。
7. 本投稿テストをする場合だけ `dry_run` を `0` にします。
